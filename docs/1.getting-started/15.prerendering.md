...
When building a Nuxt application for static hosting with `nuxt generate` or `nuxt build --prerender`, Nuxt automatically generates two special fallback HTML files alongside your prerendered routes:

- **`200.html`** — The SPA (Single-Page App) fallback. Static hosting providers serve this file for any request path that does not match a pre-rendered static file, allowing the client-side Vue Router to handle navigation. This is also known as the history API fallback or catch-all fallback.
- **`404.html`** — The static 404 error page. Hosting providers serve this file when a requested resource does not exist.

::tip
Most static hosting providers (Netlify, Vercel, GitHub Pages, Cloudflare Pages) automatically detect and use `200.html` and `404.html`. Check your provider's documentation to confirm how to configure custom fallback and error pages.
::

You can verify that both files are present in the `.output/public/` directory after running `nuxt generate`.

### Explicitly Prerendering Fallback Files

When using [hybrid rendering](/docs/4.x/guide/concepts/rendering#hybrid-rendering) with `nuxt build` (rather than `nuxt generate`), these fallback files are **not** generated automatically. You can add them explicitly via `routeRules`:
...
::read-more{to="/docs/4.x/guide/concepts/rendering#client-side-rendering"}
Read more about static client-side rendering and fallback files.
::
